...
public class EServices_Service_Request {
	...
	public String EService_Request(WebDriver driver , ExtentTest test, ExtentReports reports ,  int j , String Test_Data_Path, String Sheet,String Environment ,String Booking_Req_No ) throws Exception {
		...
		//Work Queue:Query
		Result = Element.Click_Event(driver, test, reports, "//button[@aria-label='Task Queue:Query']", "Work Queue:Query");
		...
		//Work Queue:Query -> Go
		Result = Element.Click_Event(driver, test, reports, "//button[@aria-label='Task Queue:Go']", "Work Queue:Query -> Go");
		
		//Document Reference Number text box
		Result = Element.Click_Event(driver, test, reports, "//td[@id='1_s_1_l_DC_INS_Initiation_Date']", "Document Reference Number text box");
		...
		/*
		 * Thread.sleep(5000); if (Environment.equals("UAT1")) {
		 * //wait.until(ExpectedConditions.alertIsPresent()); if
		 * (ExpectedConditions.alertIsPresent()!=null) {
