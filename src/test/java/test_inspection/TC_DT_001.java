...
import pom_dubaitrade.DubaiTrade_ServiceRequest;
...
public class TC_DT_001 {
    ...
    public static DubaiTrade_ServiceRequest DT_Service_Request;

    @Parameters({"Test_Data_Path", "Environment"})
    @Test
    public void TC_DT_01(String Test_Data_Path, String Environment) throws Exception {
        //------------------------Initialise Browser---------------------------------------------------------------------------------------------
        ...
        DT_Service_Request = new DubaiTrade_ServiceRequest();
        //------------------------Initialise Excel data read through HashMap----------------------------------------------------------------------
        ExcelApiTest Read_Data = new ExcelApiTest(Test_Data_Path);
        int NoOf_Itration = Read_Data.ExcelIteration_Count(Test_Data_Path, "Inspection_DubaiTrade");
        ...
        for (int i = 1; i <= NoOf_Itration; i++) {
            ...
            map_Inspection_DubaiTrade = Read_Exceldata_Hashmap.HashMap_Data_Inspection_DubaiTrade(i, Test_Data_Path,
                    "Inspection_DubaiTrade");
            ...
            if (map_Inspection_DubaiTrade.get("Execute").equals("NO")) {
                continue;
            }
            if (map_Inspection_DubaiTrade.get("TestCaseNo").equals("TC_DT_001")) {
                reports = Reports.reports;
                test = reports.createTest(map_Inspection_DubaiTrade.get("TestCaseDescription"));
                ...
                //-----------------------------------Enter URL-------------------------------------------------------------------------------
                driver.get(map_Environment_Details.get("DubaiTrade_URL"));
                if (Environment.equals("SIT1") || Environment.equals("SIT2")) {
                    if (map_Environment_Details.get("Browser").equals("IE")) {
                        if (driver.findElements(By.xpath("infoBlockIDImage")).size() != 0) {
                ...
                var_dtlogin = new DubaiTrade_Login();
                var_initiate_inspection_cleared = new Initiate_Inspection_Cleared();
                Thread.sleep(5000);
                // Functional Flow
                Result = var_dtlogin.DT_login(driver, test, reports, Environment, Test_Data_Path);
                ...
                Result = var_initiate_inspection_cleared.Fn_Initiate_Inspection_Cleared(driver, test, Environment, reports, i,
                        Test_Data_Path, "Inspection_DubaiTrade");
                ...
                //-------------------------------------------------Booking Planning----------------------------------------------------------
                if (map_Inspection_DubaiTrade.get("Inspection_Centre").equals("Hatta") != true) {
                    //isEmpty()==true) {
                    for (int k = 0; k <= 20; k++) {
                    ...
                    if (Planner_User.equals(map_Environment_Details.get("Planner_UserID"))) {
                        ...
                        Result = Siebel_login.Siebel_Login_Function(driver, test, reports, Environment, Test_Data_Path,
                                "Planning");
                        if (Result.equals("Pass")) {
                            test.log(Status.PASS, "Siebel Base Application logged in successfully with Planner User : "
                        ...
                        if (Planner_User.equals(map_Environment_Details.get("Planner_UserID"))) {
                            Result = INS_Planning.Planner(driver, test, reports, i, Test_Data_Path, "Inspection_DubaiTrade",
                ...
                //---------------------------Login to Application--------------------------------------------------------------------------------
                if (map_Inspection_DubaiTrade.get("Inspection_Centre").equals("Hatta")) {
                ...
                if (Result.equals("Pass")) {
                    test.log(Status.PASS, "Siebel Eservices application logged in successfully with Inspector User : "
                ...
                if (map_Inspection_DubaiTrade.get("Sieze_Flag").equals("OFF")) {
                    Result = INS_Release.Release(driver, test,
                            reports, i, Test_Data_Path, "Inspection_DubaiTrade", Environment, BookingRefNo);
                    if (Result.equals("Pass")) {
                ...
                } else if (map_Inspection_DubaiTrade.get("Sieze_Flag").equals("ON")) {
                    Result = INS_Seize.Seizure(driver, test, reports, i,
                            Test_Data_Path, "Inspection_DubaiTrade", Environment, BookingRefNo);
                    if (Result.equals("Pass")) {
